    ...
    <section className='mb-10'>
      <div className='flex justify-between items-center mb-6 mt-2 mr-6'>
        <h2 className='text-3xl font-black flex items-center'>
          <Lightbulb className='mr-2' /> Skills
        </h2>
        <Button
      ...
      <div className='overflow-x-auto pb-4 -mx-4 px-4 pt-4' style={{ overflowY: 'hidden' }}>
        <div className='flex gap-6 min-w-max'>
          {profile.skills.map((skill, index) => (
            <div
              key={index}
              className='w-[200px] h-[200px] bg-white border-[3px] border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center p-4 hover:translate-y-[-5px] hover:shadow-[5px_10px_0px_0px_rgba(0,0,0,1)] transition-all'
            >
              <div className='w-24 h-24 rounded-full border-[3px] border-black bg-[#f8f8f8] flex items-center justify-center mb-3 relative'>
                ...
                <svg className='absolute inset-0' width='100%' height='100%' viewBox='0 0 100 100'>
                  ...
                  <circle
                    ...
                    fill='none'
                    stroke={
                      skill.level >= 80 ? '#8ac926' : skill.level >= 60 ? '#4cc9f0' : '#ff6b6b'
                    }
                    strokeWidth='8'
